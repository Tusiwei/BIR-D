...
class GaussianDiffusion:
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, pred_xstart, t, light_factor=None, light_variance=None, model_kwargs=None):
        """
        ...
        for i in range(6):
            light_factor, light_variance, gradient = cond_fn(pred_xstart, self._scale_timesteps(t), light_factor=light_factor, light_variance=light_variance,  **model_kwargs)
    ...
    def p_sample(
        ...
        if cond_fn is not None:
            for i in range(1):
                out["mean"], light_factor, light_variance = self.condition_mean(
                    cond_fn, out, x, out["pred_xstart"], t, light_factor = light_factor, light_variance = light_variance, model_kwargs=model_kwargs
                )
        sample = out["mean"] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise
        return {"sample": sample, "pred_xstart": out["pred_xstart"], "light_factor": light_factor, 'light_variance':light_variance}, light_factor, light_variance
    ...
    def p_sample_loop(
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            final = sample
        return final["sample"], final["light_factor"], final["light_variance"]
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                out, light_factor, light_variance = self.p_sample(
                    model,
                    img,
                    t,
                    light_factor,
