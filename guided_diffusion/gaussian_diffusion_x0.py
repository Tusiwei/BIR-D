...
class GaussianDiffusion:
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, pred_xstart, t,weight=None,light_mask=None,guidance_scale=None,model_kwargs=None):
        """
        ...
        for i in range(1):
            weight,light_mask,gradient,mse = cond_fn(pred_xstart, self._scale_timesteps(t),weight=weight,light_mask=light_mask,guidance_scale=guidance_scale, **model_kwargs)
    ...
    def p_sample(
        ...
        if cond_fn is not None:
            for i in range (2):
                out["mean"], weight_tmp,light_mask_tmp,gradient,mse = self.condition_mean(
                    cond_fn, out, x, out["pred_xstart"], t,weight=weight,light_mask = light_mask,guidance_scale=guidance_scale, model_kwargs=model_kwargs
                )
        
        # mse = mse / (3 * p_size * p_size)
                
        new_out_mean += out["mean"]   
        weight_return += weight_tmp
        light_mask_return += light_mask_tmp
        new_out_pred_xstart += out["pred_xstart"]
        new_out_log_variance += out["log_variance"]

        sample = new_out_mean + nonzero_mask * th.exp(0.5 * new_out_log_variance) * noise
        
        '''
        in_channels = 3
        # padding=1表示填充一圈0,kernel_size应与卷积核weight的大小匹配
        ...
        guidance_scale = - guidance_scale.abs() / 9
    ...
    # bqd表示bit per dimension，用于评估生成模型的性能
    def _vb_terms_bpd(
